...
public class AppStoreTest {
    ...
    void setUp() {
        ...
        edAppBelowBoundary = new EducationApp(developerLego, "WeDo", 1, 1.0, 0, CategoryUtility.getDefaultSubjectCategory(), 1);

        edAppOnBoundary = new EducationApp(developerLego, "Spike", 1000, 2.0,
                1.99, "Technology", 10);

        edAppAboveBoundary = new EducationApp(developerLego, "EV3", 1001, 3.5,  2.99, "Business", 11);
        ...
        prodAppBelowBoundary = new ProductivityApp(developerApple, "NoteKeeper", 1, 1.0, 0.0, CategoryUtility.getDefaultProductivityCategory());

        prodAppOnBoundary = new ProductivityApp(developerMicrosoft, "Outlook", 1000, 2.0, 1.99, "Mail");

        prodAppAboveBoundary = new ProductivityApp(developerApple, "Pages", 1001, 3.5, 2.99, "Utility");

        prodAppInvalidData = new ProductivityApp(developerMicrosoft, "", -1, 0, -1.00, "");


        //Validation: appSize(1-1000), appVersion(>=1.0), ageRating (0-18), appCost(>=0),
        gameAppBelowBoundary = new GameApp(developerEAGames, "Tetris", 1, 1.0, 0.0, CategoryUtility.getDefaultGameCategory(), false);

        gameAppOnBoundary = new GameApp(developerKoolGames, "CookOff", 1000, 2.0, 1.99, "Strategy", true);

        gameAppAboveBoundary = new GameApp(developerEAGames, "Empires", 1001, 3.5,  2.99, "Action", false);
