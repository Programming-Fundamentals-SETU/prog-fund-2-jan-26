...
public class EducationAppTest {
    ...
    void setUp() {
        ...
        edAppBelowBoundary = new EducationApp(developerLego, "WeDo", 1, 1.0, 0,
                CategoryUtility.getDefaultSubjectCategory(), 1);

        edAppOnBoundary = new EducationApp(developerLego, "Spike", 1000, 2.0,
                 1.99, "Technology", 10);

        edAppAboveBoundary = new EducationApp(developerLego, "EV3", 1001, 3.5,
                2.99, "Business", 11);

        edAppInvalidData = new EducationApp(developerLego, "", -1, 0, -1.00,
                 "", 0);
    }
    ...
    class RecommendedApp {
        ...
        void appIsNotRecommendedWhenCategoryIsGeneral() {
            //setting all conditions to true ratings of 3 and 4 (i.e. 3.5)
            EducationApp edApp = setupEducationAppWithRating(3, 4);
            edApp.setCategory(CategoryUtility.getDefaultSubjectCategory());   //setting to General
            //verifying recommended app returns false (wrong subject category)
            assertFalse(edApp.isRecommendedApp());
        }
