                ...
                switch (option) {
                    case 1 -> {
                        ...
                        if (CategoryUtility.isValidGameCategory(gameGenre)) {
                            appStoreAPI.addApp(new GameApp(developer, appName, appSize, appVersion, appCost, gameGenre, multiplayer));
                        } else {
                    ...
                    case 2 -> {
                        ...
                        if (CategoryUtility.isValidSubjectCategory(subjectCategory)) {
                            appStoreAPI.addApp(new EducationApp(developer, appName, appSize, appVersion, appCost, subjectCategory, level));
                        } else {
                    ...
                    case 3 -> {
                        ...
                        if (CategoryUtility.isValidProductivityCategory(productivityCategory)) {
                            appStoreAPI.addApp(new ProductivityApp(developer, appName, appSize, appVersion, appCost, productivityCategory));
                        } else {
    ...
    private void searchAppsBySpecificCriteria() {
        System.out.println("""
                What criteria would you like to search apps by:
                  ...
                  2) Developer Name
                  3) Category
                  4) Rating (all apps above it)""");
        int option = ScannerInput.validNextInt("==>> ");
        switch (option) {
            case 1 -> searchAppsByName();
            case 2 -> searchAppsByDeveloper(readValidDeveloperByName());
            case 3 -> searchAppsByCategory();
            case 4 -> searchAppsAboveAStarRating();
            default -> System.out.println("Invalid option");
        }
    }
