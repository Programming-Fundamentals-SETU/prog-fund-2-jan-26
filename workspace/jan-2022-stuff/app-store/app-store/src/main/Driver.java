...
public class Driver {
    ...
    private void runMainMenu() {
        ...
        while (option != 0) {
            switch (option) {
                ...
                case 8 -> simulateAppDownloads();
                case 9 -> simulateRatings();
                case 20 -> saveAllData();
                case 21 -> loadAllData();
        ...
        while (option != 0) {
            switch (option) {
             ...
             //   case 2 -> System.out.println(developerAPI.listDevelopers());
                case 2 -> updateDeveloper();
                case 3 -> deleteDeveloper();
                default -> System.out.println("Invalid option entered" + option);
            }
            ScannerInput.validNextLine("\n Press the enter key to continue");
    ...
    private void addApp() {
        ...
        if (developer != null) {
            ...
            if (!appStoreAPI.isValidAppName(appName)) {
                ...
                boolean inAppPurchase = Utilities.YNtoBoolean(
                        ScannerInput.validNextChar("\tIn app purchase (Y/N): "));
                String license = readValidLicense();
                ...
                switch (option) {
                    case 1 -> {
                        //Games App
                        boolean multiplayer = Utilities.YNtoBoolean(
                                ScannerInput.validNextChar("\tMultiplayer (Y/N): "));
                        String gameGenre = ScannerInput.validNextLine("\tGames category options " + CategoryUtility.getGameCategories() + ": ");
                        if (CategoryUtility.isValidGameCategory(gameGenre)) {
                            appStoreAPI.addApp(new GameApp(developer, appName, appSize, appVersion, ageRating, inAppPurchase, license, appCost, gameGenre, multiplayer));
                        } else {
                            appStoreAPI.addApp(new GameApp(developer, appName, appSize, appVersion, ageRating, inAppPurchase, license, appCost, "", multiplayer));
                        }
                    }
                    case 2 -> {
                        //Education App
                        int level = ScannerInput.validNextInt("\tLevel (1-10): ");
                        String subjectCategory = ScannerInput.validNextLine("\tSubject category options " + CategoryUtility.getProductivityCategories() + ": ");
                        if (CategoryUtility.isValidSubjectCategory(subjectCategory)) {
                            appStoreAPI.addApp(new EducationApp(developer, appName, appSize, appVersion, ageRating, inAppPurchase, license, appCost, subjectCategory, level));
                        } else {
                            appStoreAPI.addApp(new EducationApp(developer, appName, appSize, appVersion, ageRating, inAppPurchase, license, appCost, "", level));
                        }
                    }
                    case 3 -> {
                        //Productivity App
                        String productivityCategory = ScannerInput.validNextLine("\tProductivity category options " + CategoryUtility.getSubjectCategories() + ": ");
                        if (CategoryUtility.isValidProductivityCategory(productivityCategory)) {
                            appStoreAPI.addApp(new ProductivityApp(developer, appName, appSize, appVersion, ageRating, inAppPurchase, license, appCost, productivityCategory));
                        } else {
                            appStoreAPI.addApp(new ProductivityApp(developer, appName, appSize, appVersion, ageRating, inAppPurchase, license, appCost, ""));
                        }
    ...
    private void searchAppsBySpecificCriteria() {
        System.out.println("""
                What criteria would you like to search apps by:
                  ...
                  3) Licence Type
                  4) Category
                  5) Rating (all apps above it)
                  6) Age Rating (all apps below it)""");
        int option = ScannerInput.validNextInt("==>> ");
        switch (option) {
            case 1 -> searchAppsByName();
            case 2 -> searchAppsByDeveloper(readValidDeveloperByName());
            case 3 -> System.out.println(appStoreAPI.listAllAppsByLicenseType(readValidLicense()));
            case 4 -> searchAppsByCategory();
            case 5 -> searchAppsAboveAStarRating();
            case 6 -> searchAppsBelowAgeRating();
            default -> System.out.println("Invalid option");
        }
    }
    ...
    private void searchAppsByCategory() {
        //todo bug fix
        System.out.println("Games Categories: " + CategoryUtility.getGameCategories());
        System.out.println("Educational Categories: " + CategoryUtility.getSubjectCategories());
        System.out.println("Productivity Categories: " + CategoryUtility.getProductivityCategories());
        String category = ScannerInput.validNextLine("Which category do you want to search by: ");
        System.out.println(appStoreAPI.listAllAppsByCategory(category));
    }
    ...
    private void searchAppsBelowAgeRating() {
        int rating = ScannerInput.validNextInt("View apps whose age rating is lower than: ");
        System.out.println(appStoreAPI.listAllAppsBelowAgeRating(rating));
    }


    private void sortApps() {
        System.out.println("""
                What criteria would you like to sort apps by:
                  1) App Name Ascending
                  2) Age Rating Ascending
                  3) Number of Downloads Descending""");
        int option = ScannerInput.validNextInt("==>> ");
        switch (option) {
            case 1 -> {
            ...
            case 2 -> {
                System.out.println("Sorting Apps by age rating ascending...");
                appStoreAPI.sortAppsByAgeRatingAscending();
            }
            case 3 -> {
                System.out.println("Sorting Apps by number of downloads descending...");
                appStoreAPI.sortAppsByNumberOfDownloadsDescending();
            }
            default -> System.out.println("Invalid option");
        }
    }
    ...
    private void simulateAppDownloads() {
        // simulate random app downloads for all apps (to give data for recommended apps and reports etc).
        if (appStoreAPI.numberOfApps() > 0) {
            System.out.println("Simulating app downloads between 0 and 1000...");
            appStoreAPI.simulateAppDownloads();
            System.out.println(appStoreAPI.listSummaryOfAllApps());
        } else {
            System.out.println("No apps");
        }
    ...
    //  "Get" Helper Methods
    //--------------------------------------------------
    private String readValidLicense() {
    ...
    private String readValidAppName() {
        String appName = ScannerInput.validNextLine("\tApp Name: ");
        if (appStoreAPI.isValidAppName(appName)) {
            return appName;
        } else {
