...
*In this section, you may need to refer to your lecture slides for support (i.e. in Topic 7).*

Within your ShopV7.0 project, open Store.java.  Click above the **public class Store** declaration and when the light bulb appears, click on it and select **Create Test**.  Choose to generate the following CRUD methods:
...
~~~
You will notice that you will get errors on Product. This is because the test class is not in the same package as the class being tested. 
Update the import section with :
~~~
import models.Product;
~~~

This will remove the errors.
If any other such errors appear, fix them using the IntelliJ suggestions.
