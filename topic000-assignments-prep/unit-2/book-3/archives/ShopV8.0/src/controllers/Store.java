...
public class Store {
    ...
    /**
     * Selection sort algorithm for  sorting the arraylist of products by product name ascending.
     */
    public void sortProductsByNameAscending()
    {
        for (int i = products.size() -1; i >= 0; i--)
        {
            ...
            swapProducts(i, highestIndex);
        }
    }

    public void sortProductsByUnitCostAscending() {
        for (int i = products.size() - 1; i >= 0; i--) {
            int highestIndex = 0;
            for (int j = 0; j <= i; j++) {
                if (products.get(j).getUnitCost() > products.get(highestIndex).getUnitCost()) {
                    highestIndex = j;
                }
            }
            swapProducts(i, highestIndex);
    ...
    private void swapProducts( int i, int j) {
