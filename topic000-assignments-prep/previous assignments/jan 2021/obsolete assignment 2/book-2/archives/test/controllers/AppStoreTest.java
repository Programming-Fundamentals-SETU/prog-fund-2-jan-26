...
public class AppStoreTest {
    ...
    private Developer developerEAGames = new Developer("EA Games", "http://www.eagames.com");
    private Developer developerKoolGames = new Developer("Kool Games", "http://www.koolgames.com");
    private Developer developerApple = new Developer("Apple", "http://www.apple.com");
    private Developer developerMicrosoft = new Developer("Microsoft", "http://www.microsoft.com");
    ...
    void setUp() {
        ...
        prodAppBelowBoundary = new ProductivityApp(developerApple, "NoteKeeper", 1, 1.0, 0, false,
                "Noncommercial", 0, CategoryUtility.getDefaultProductivityCategory());

        prodAppOnBoundary = new ProductivityApp(developerMicrosoft, "Outlook", 1000, 2.0, 18, true,
                "Proprietary", 1.99, "Mail");

        prodAppAboveBoundary = new ProductivityApp(developerApple, "Pages", 1001, 3.5, 19, true,
                "Trade secret", 2.99, "Utility");

        prodAppInvalidData = new ProductivityApp(developerMicrosoft, "", -1, 0, -1, false,
                "", -1.00, "");


        //Validation: appSize(1-1000), appVersion(>=1.0), ageRating (0-18), appCost(>=0),
        gameAppBelowBoundary = new GameApp(developerEAGames, "Tetris", 1, 1.0, 0, false,
                "Noncommercial", 0, CategoryUtility.getDefaultGameCategory(), false);

        gameAppOnBoundary = new GameApp(developerKoolGames, "CookOff", 1000, 2.0, 18, true,
                "Proprietary", 1.99, "Strategy", true);

        gameAppAboveBoundary = new GameApp(developerEAGames, "Empires", 1001, 3.5, 19, true,
                "Trade secret", 2.99, "Action", false);

        gameAppInvalidData = new GameApp(developerKoolGames, "", -1, 0, -1, false,
                "", -1.00, "", true);
    ...
    class ListingMethods {
        ...
        void listRecommendedAppsReturnsRecommendedAppsWhenTheyExist() {
            ...
            appStore.addApp(setupGameAppWithRatingAndDownloads(100,5,4));
            appStore.addApp(setupEducationAppWithRatingAndDownloads(5,3,4));
            appStore.addApp(setupProductivityAppWithRatingAndDownloads(10, 3, 4));
            assertEquals(10, appStore.numberOfApps());

            String apps = appStore.listAllRecommendedApps();
            //checks for the three objects in the string
            assertTrue(apps.contains("LegoMaths"));
    ...
    EducationApp setupEducationAppWithRatingAndDownloads(int numberDownloads, int rating1, int rating2) {
        ...
        //      inAppPurchase is false; appCost is > .99; numberofDownloads >= 5; rating of >= 3.5
        //      category is not the default of General
        EducationApp edApp = new EducationApp(developerLego, "LegoMaths", 1,
                1.0, 0, false, "Noncommercial",
                1.00, "Maths", 1);
        edApp.setNumberOfDownloads(numberDownloads);
        edApp.addRating(new Rating(rating1, "John Doe", "Learned a lot"));
        edApp.addRating(new Rating(rating2, "Jane Doe", "Excellent"));
        return edApp;
    }

    GameApp setupGameAppWithRatingAndDownloads(int numberDownloads, int rating1, int rating2) {
        // Algorithm - app is recommended if the following applies:
        //      inAppPurchase is true;  numberofDownloads >= 100; rating of >= 4.0
        GameApp gameApp = new GameApp(developerEAGames, "MazeRunner", 1,
                1.0, 0, true, "Noncommercial",
                1.00, "Strategy", true);
        gameApp.setNumberOfDownloads(numberDownloads);
        gameApp.addRating(new Rating(rating1, "John Soap", "Exciting Game"));
        gameApp.addRating(new Rating(rating2, "Jane Soap", "Nice Game"));
        return gameApp;
    }

    ProductivityApp setupProductivityAppWithRatingAndDownloads(int numberDownloads, int rating1, int rating2) {
        // Algorithm - app is recommended if the following applies:
        //      inAppPurchase is false; appCost is >= 1.99; rating of > 3.0; category is not the default of Utility
        ProductivityApp productivityApp = new ProductivityApp(developerApple, "Evernote", 1,
                1.0, 0, false, "Noncommercial",
                1.99, "Notes");
        productivityApp.setNumberOfDownloads(numberDownloads);
