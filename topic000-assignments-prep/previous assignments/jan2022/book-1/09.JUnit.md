...
- *ItemTest*: This class exhaustively tests the methods in the Item class. 
...
The test class for Item is  available [here](archives/test/ItemTest.java). 
...
Note that the *src* folder is blue in colour and the *test* one is not:
...
- We use the Item class as it is the simplest and the best one for you to start with. 

- Write the Item class in the normal way.

- Drag in the ItemTest class as supplied above  into the test directory.
...
Click on the  lightbulb that appears and then select *Add Junit5xx to Classpath* (xx stands for latest version) :

![](img/test-06.png)

Then you need to agree to download that library:

![](img/test-07.png)

Press OK and this should work.
...
(If the annotations still contain errors, accept the import suggestion from *Context Actions*)
